...
Rhino.require('alt.util.Inspector', true);
...
InspectorFunction.prototype.toHTML = function(name) {
	...
	return <dl style="margin:0">
	         <dt>
	          <code>
	           <b><a href="#" onclick={toggleCode('span'+id)}>{name}</a></b>({args})
	           </code>
	          </dt>
	         <dd>{shortdesc}
...
InspectorClass.prototype.toHTML = function(name) {
	...
	var s = <p>
	<p><code>class <b>{name}</b><br/>
	{this.func.constructor}<br/>
	{this.func.prototype}<br/>
	{this.func.__proto__}<br/>
	{ this.func.prototype ? this.func.prototype.constructor : ''}<br/>
	{ this.func.prototype ? this.func.prototype.__proto__ : ''}<br/>
	{this.func.__proto__ ? this.func.__proto__.constructor : ''}<br/>
	{extendsClass}</code></p>
	<p>{classDesc}</p>
	<dl>
	{since}
	...
	if (this.func instanceof Function) {
		...
		s.appendChild(<p>{table}</p>);
	}
	if (this.children)
		s.appendChild(InspectorObject.prototype.toHTML.call(this,name));
	if (this.proto) {
		s.appendChild(this.proto.toHTML(name,'Member'));
		if (superClass) {
			var overridden = {};
			var node = this;
